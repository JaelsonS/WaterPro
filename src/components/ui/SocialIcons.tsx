...
export function WhatsAppIcon({ className = "w-5 h-5" }: SocialIconProps) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    ...
    <div className={`flex items-center gap-3 ${className}`}>
      {socialLinks.map(({ href, label, Icon }) => (
        <a
          ...
          aria-label={label}
          className="group flex h-11 w-11 items-center justify-center rounded-full bg-white border border-slate-line text-ink-muted transition-all duration-300 hover:border-azure/40 hover:text-azure hover:shadow-[0_4px_20px_rgba(26,127,184,0.12)] hover:-translate-y-0.5"
        >
