...
import type { EnterpriseApplication, EnterpriseCategory } from "@/data/enterprise/types";
import {
  allEnterpriseApplications,
  allEnterpriseCategories,
  enterpriseApplicationLabels,
  enterpriseCategoryLabels,
} from "@/data/enterprise/categories";
import { filterEnterpriseProducts } from "@/data/enterprise/products";
import { EnterpriseProductCard } from "./EnterpriseProductCard";
import { cn } from "@/lib/utils";

export function EnterpriseCatalog() {
  const locale = useLocale() as "pt" | "en";
  const [category, setCategory] = useState<EnterpriseCategory | "all">("all");
  const [application, setApplication] = useState<EnterpriseApplication | "all">("all");
  const [search, setSearch] = useState("");
  const [sort, setSort] = useState<"name" | "featured">("featured");

  const filtered = useMemo(
    () => filterEnterpriseProducts({ category, application, search, sort }),
  ...
  return (
    <div>
      <div className="mb-8">
        <div className="relative">
          ...
          <input
            ...
            placeholder={
              locale === "pt" ? "Pesquisar equipamentos..." : "Search equipment..."
            }
            ...
            aria-label={locale === "pt" ? "Pesquisar equipamentos" : "Search equipment"}
          />
        </div>
      </div>
      ...
      <div className="mb-8 flex flex-wrap items-center gap-3">
        ...
        {allEnterpriseCategories.map((cat) => (
          <button
            key={cat}
      ...
      <div className="mb-8 flex flex-wrap items-center gap-2">
        ...
        {allEnterpriseApplications.map((app) => (
          <button
            key={app}
      ...
      <div className="mb-8 flex items-center justify-between">
        <p className="text-sm text-ink-muted">
          {filtered.length}{" "}
          {locale === "pt" ? "equipamentos encontrados" : "products found"}
        </p>
        <select
          value={sort}
          ...
          aria-label={locale === "pt" ? "Ordenar equipamentos" : "Sort equipment"}
        >
      ...
      {filtered.length === 0 ? (
        <div className="py-20 text-center">
          <p className="text-ink-muted">
            {locale === "pt" ? "Nenhum equipamento encontrado." : "No equipment found."}
          </p>
        </div>
      ) : (
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((product) => (
            <EnterpriseProductCard key={product.slug} product={product} />
          ))}
        </div>
      )}
    </div>
